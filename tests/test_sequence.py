...
def test_scenario():
    ...
    print(f'Number of open issues: {len(open_issues1)}')

    # step 2
    practice1_issues = get_issues({'labels': ['practice1']})
    print(f'Number of issues labeled "practice1": {len(practice1_issues)}')
    ...
    # step 4
    print(f"Number of issue created: {created_issue['number']}")
    ...
    # step 6
    _ = update_issue(issue_num=created_issue['number'], state='closed', state_reason='not_planned')
